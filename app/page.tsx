...
export default function BiyoraHome() {
  ...
  const handlePaystackPayment = async () => {
    ...
    if (!process.env.NEXT_PUBLIC_PAYSTACK_PUBLIC_KEY) {
      setToast({ message: "Payment configuration error. Check .env.local", type: "error" });
      return;
    }

    // Safe Paystack script check
        ...
        onSuccess: async (transaction: any) => {
          ...
          try {
            ...
            if (verifyData.success) {
              ...
              setToast({ 
                message: "Payment received but verification failed. Contact support with reference.", 
                type: "error" 
              });
            }
          } catch (verifyError) {
            ...
            setToast({ 
              message: "Payment successful but verification pending. Save your reference.", 
              type: "info" 
            });
          }
        },
        onCancel: () => setToast({ message: "Payment was cancelled", type: "info" }),
        onError: () => setToast({ message: "Payment error occurred. Please try again.", type: "error" })
      });
    } catch (error) {
      console.error("Paystack initialization error:", error);
      setToast({ message: "Failed to initialize payment. Please refresh and try again.", type: "error" });
    } finally {
