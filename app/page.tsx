...
export default function BiyoraHome() {
      ...
      paystack.newTransaction({
        ...
        onSuccess: async (transaction: any) => {
          ...
          if (verifyData.success) {
            setToast({ 
              message: `🎉 Order confirmed! Reference: ${transaction.reference}`, 
              type: "success" 
            });
            clearCart();
            setEmail('');
            ...
            setToast({ 
              message: "Payment received but verification failed. Contact support.", 
              type: "error" 
            });
          }
        },
        onCancel: () => setToast({ message: "Payment cancelled", type: "info" }),
        onError: () => setToast({ message: "Payment error. Try again.", type: "error" })
      });
    } catch (error) {
      console.error(error);
  ...
  return (
    <main className="min-h-screen bg-gray-950 text-white">
      {/* Header, Hero, Products Grid - same as before */}
      ...
      {/* Hero Section */}
      <div className="relative h-[70vh] flex items-center justify-center overflow-hidden hero-bg">
        <div className="relative z-10 text-center px-6 max-w-4xl">
          ...
          <button 
            ...
            className="bg-[#d4af37] hover:bg-amber-300 text-black font-semibold px-12 py-5 rounded-2xl text-lg transition-all"
          >
            Shop Collection
          </button>
        </div>
      ...
      {/* Floating Cart */}
      {cart.length > 0 && (
        <div className="fixed bottom-6 right-6 bg-gray-900 border border-[#d4af37]/30 p-6 rounded-3xl shadow-2xl max-w-xs w-full z-50 floating-cart">
          ...
          <button
            ...
            className="w-full bg-[#d4af37] hover:bg-amber-300 disabled:bg-gray-600 text-black font-semibold py-4 rounded-2xl transition-all"
          >
            {isVerifying ? 'Verifying...' : isProcessing ? 'Processing...' : 'Pay Securely with Paystack'}
          </button>
        </div>
      )}
