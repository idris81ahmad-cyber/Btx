      ...
      <section id="products" className="max-w-7xl mx-auto px-6 py-20">
          ...
          <div className="flex flex-wrap gap-2">
            {categories.map((category) => (
              <button
                ...
                className={`px-6 py-3 rounded-3xl text-sm font-medium transition-all border active:scale-[0.985] ${selectedCategory === category
            ...
            {(searchQuery || selectedCategory !== 'All') && (
              <button
                ...
                className="px-5 py-3 rounded-3xl text-sm border border-red-500/40 text-red-400 hover:bg-red-500/10 active:scale-[0.985] transition-all"
              >
                Clear
              </button>
            )}
        ...
        {filteredProducts.length > 0 ? (
          ...
          <div className="text-center py-20">
            ...
            <button onClick={clearFilters} className="mt-6 px-8 py-3 rounded-3xl border border-[#d4af37]/40 hover:bg-[#d4af37]/10 transition-all">
              Clear filters
            </button>
          </div>
        )}
      ...
      {(cart.length > 0 || isCartOpen) && (
        <div className="fixed bottom-6 right-6 bg-zinc-900 border border-[#d4af37]/30 p-7 rounded-3xl shadow-2xl max-w-sm w-full z-50">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={() => setIsCartOpen(false)} className="text-[#d4af37]/70 hover:text-white text-2xl">×</button>
          </h3>

          {cart.length > 0 ? (
            <>
              <div className="max-h-[200px] overflow-auto mb-6 space-y-4 text-sm pr-1">
                {cart.map((item) => (
              ...
              >
                {isVerifying ? 'Verifying...' : isProcessing ? 'Processing...' : 'Pay with Paystack'}
              </button>
            </>
          ) : (
